...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') {
      ...
      session.modifyConfig('displayCurrency', 'XTE');
      eventEmitter.emit('modifyCurrency', 'XTE');
    }
    ReactTooltip.rebuild();
  };
  ...
  render() {
    ...
    if (session.wallet && displayCurrency === 'XTE') {
      balanceTooltip =
        `Unlocked: ${session.atomicToHuman(unlockedBalance, true)} ${
          il8n.XTE
        }<br>` +
        `Locked: ${session.atomicToHuman(lockedBalance, true)} ${il8n.XTE}`;
    } else if (
      session.wallet &&
