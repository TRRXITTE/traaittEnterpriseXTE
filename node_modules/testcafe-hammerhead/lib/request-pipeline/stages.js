...
exports.default = [
    ...
    async function sendDestinationRequest(ctx) {
        ctx.reqOpts = request_options_1.default.createFrom(ctx);
        ...
        if (ctx.session.hasRequestEventListeners()) {
            ...
            await ctx.forEachRequestFilterRule(async (rule) => {
                const requestEvent = await (0, utils_1.callOnRequestEventCallback)(ctx, rule, requestInfo);
                ctx.setupMockIfNecessary(requestEvent);
            });
        }
        if (ctx.mock) {
            await ctx.mockResponse();
            await (0, utils_1.handleRequestMockingErrorIfNecessary)(ctx);
        }
        else
    ...
    async function checkSameOriginPolicyCompliance(ctx) {
        ...
        await ctx.forEachRequestFilterRule(async (rule) => {
            ...
            await (0, utils_1.callOnResponseEventCallbackForFailedSameOriginCheck)(ctx, rule, configure_response_event_options_1.default.DEFAULT);
        });
    ...
    async function decideOnProcessingStrategy(ctx) {
        ...
        // NOTE: Just pipe the content body to the browser if we don't need to process it.
            ...
            await (0, utils_1.callOnConfigureResponseEventForNonProcessedRequest)(ctx);
            ctx.sendResponseHeaders();
            if (ctx.contentInfo.isNotModified)
                return await (0, utils_1.callOnResponseEventCallbackForMotModifiedResource)(ctx);
            ...
            if (onResponseEventDataWithBody.length)
                await (0, utils_1.callOnResponseEventCallbackWithBodyForNonProcessedRequest)(ctx, onResponseEventDataWithBody);
            else if (onResponseEventDataWithoutBody.length)
                await (0, utils_1.callOnResponseEventCallbackWithoutBodyForNonProcessedResource)(ctx, onResponseEventDataWithoutBody);
    ...
    async function sendProxyResponse(ctx) {
        ...
        (0, connection_reset_guard_1.connectionResetGuard)(async () => {
            await Promise.all(configureResponseEvents.map(async (configureResponseEvent) => {
                await (0, utils_1.callResponseEventCallbackForProcessedRequest)(ctx, configureResponseEvent);
            }));
